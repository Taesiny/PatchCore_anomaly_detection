...
class Umap_data(Dataset):
    def __init__(self, embeddings, shrinking_factor):
        ...
        print(f'n_neighbors: {args.umap_n_neighbors}\nmetric: {args.umap_metric}\nmin_dist: {args.umap_min_dist}\nn_components: {int(embeddings.shape[1]*args.shrinking_factor)}')
    ...
    def training_epoch_end(self, outputs): 
        ...
        if args.umap:
            ...
            self.dim_reducer = umap.UMAP(
                        n_neighbors=int(args.umap_n_neighbors),
                        n_components= int(total_embeddings.shape[1]*shrinking_factor),
                        metric=str(args.umap_metric),
                        min_dist=float(args.umap_min_dist),
                        spread=float(args.umap_spread),
                        target_metric='l2',
            ...
            print(f'n_neighbors: {args.umap_n_neighbors}\nmetric: {args.umap_metric}\nmin_dist: {args.umap_min_dist}\nn_components: {int(total_embeddings.shape[1]*args.shrinking_factor)}')
    ...
    def prediction_process_core_piecwise_measuremet(self, batch, batch_idx):
        ...
        if x.size()[0] <= 1:
            ...
            if args.umap:
                ...
                print('\nTransform Features started...')
                # temp_flatten = np.reshape(embedding_test,(embedding_test.shape[0]*embedding_test.shape[1], embedding_test.shape[2]))
                # transformed_flatten = self.nn_umap(torch.Tensor(temp_flatten).to('cuda'))
                # embedding_test = np.reshape(transformed_flatten,(embedding_test.shape[0], embedding_test.shape[1], int(embedding_test.shape[2]*args.shrinking_factor)))
                embedding_test = self.nn_umap(torch.Tensor(embedding_test).to('cuda')).cpu().numpy()
                print('... finished!')
            elif args.pca:
    ...
    def test_step(self, batch, batch_idx): # Nearest Neighbour Search
        ...
        if self.measure_latences:
            ...
            for this_entry in run_times.items():
                # print(this_entry)
                # print(f'len(this_entry[1]) {len(this_entry[1])} \nbatch[0].size() {batch[0].size()}')
...
if __name__ == '__main__':
    ...
    for k in range(1000):
        ...
        args.__dict__['umap_nn'] = True
        args.__dict__['shrinking_factor'] = float(round(np.random.ranf()*0.8,2))
        args.__dict__['umap_metric'] = 'cosine'
        min_dist = float(np.random.ranf()*4)
        args.__dict__['umap_min_dist'] = round(min_dist,2)
        args.__dict__['umap_spread'] = round(min_dist * (1+np.random.ranf()*2),2)
        args.__dict__['umap_n_neighbors'] = np.random.randint(2,100)
        
        for key in summary_dict.keys():
            if not key.__contains__('auc'):
                summary_dict[key] += [args.__dict__[key]]
        ...
        summary_dict['img_auc'] += [float(img_auc)]
        summary_dict['pixel_auc'] += [float(pixel_auc)]
        
        pd.DataFrame(summary_dict).to_csv(f'hyperop_random_umap_nn_after_run_{k}_{id}.csv')
